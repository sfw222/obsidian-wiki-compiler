...
- **概念提取** — 自动提取跨文章的高频概念，通过 SearXNG 搜索权威信息，生成专属概念页面；所有页面生成完毕后统一注入双向链接；已有概念页面通过代码侧过滤跳过，无额外 LLM token 消耗
...
- **操作日志** — `_log.md` 记录所有操作（编译、问答、检查、概念提取）；每次编译的详细步骤日志按时间戳保存至 `_runs/`
...
6. **增强** — 若配置了 SearXNG，提取高频概念并通过网络搜索生成专属概念页面；已有概念自动跳过
7. **日志** — 每次编译的逐步详情以时间戳命名保存至 `_runs/`
