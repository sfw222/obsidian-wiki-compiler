...
- **Wiki 健康检查** — 两阶段检查：(1) 代码级静态分析（零 LLM 消耗）— 检测缺失 `source` 字段、无来源的 facts/relations、超期未复审文章（自动标记 `needs-review`）、孤立页面；(2) LLM 分析矛盾内容、缺失概念和过时信息
